...
<body id="page-top">
    ...
    <!-- Top Courses -->
        ...
        <a href="new_course.php" class="btn mx-5 btn-success">+ New course</a>
        <div class="card-group d-flex justify-content-left">
            ...
            global $start, $pages;
            if($search != "") {
                $query = mysqli_query($connection, "SELECT *, course.id AS id_course FROM course INNER JOIN user ON course.user_id = user.id WHERE course.course_name like '%".$search."%' && course.company = '$username'");
            } else {
                $query = mysqli_query($connection, "SELECT *, course.id AS id_course FROM course INNER JOIN user ON course.user_id = user.id WHERE course.company = '$username' ORDER BY course_name ASC");
                $perPage = 6;
                $page = isset($_GET['page']) ? (int) $_GET['page'] : 1;
                $start = $perPage*($page-1);
                $get = mysqli_fetch_array(mysqli_query($connection, "SELECT COUNT(*) total FROM course INNER JOIN user ON course.user_id = user.id WHERE course.company = '$username'"));
                $total = $get['total'];
                $pages = ceil($total/$perPage);
                $query = mysqli_query($connection, "SELECT *, course.id AS id_course FROM course INNER JOIN user ON course.user_id = user.id  WHERE course.company = '$username' ORDER BY course_name ASC LIMIT $start, $perPage");
            }
                        ...
                        <div class="col md-4">
                            <div class="card m-5 border border-2 rounded" style="width: 19rem;">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="fs-6">Total clicked: <?php echo $row['total_clicked']?></p>
                                    <p class="mt-3"><?php
